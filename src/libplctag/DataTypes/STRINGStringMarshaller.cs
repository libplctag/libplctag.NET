...
{
    public class STRINGStringMarshaller: IMarshaller<string>
    {
        ...
        public string Decode(Tag tag, int index)
        {

            // The first two bytes of a STRING tag encode its length.
            var apparentStringLength = tag.GetInt32(index * ElementSize);
            ...
            for (int i = 0; i < stringLength; i++)
            {
                ...
                returnString.Append(Convert.ToChar(tag.GetUInt8(index * ElementSize + i + 2 + 2)));
            }

            return returnString.ToString();
        }

        public void Encode(Tag tag, int index, string value)
        {
            ...
            tag.SetInt32(index * ElementSize, value.Length);

            for (int i = 0; i < valueToWrite.Length; i++)
            {
                tag.SetUInt8(index * ElementSize + i + 2 + 2, Convert.ToByte(valueToWrite[i]));
            }
        }
    }
}
